# dsh-usage-stats — DeepSeek Harness 消耗统计插件

[![dsh-plugin](https://img.shields.io/badge/topic-dsh--plugin-blue)](https://github.com/topics/dsh-plugin)

一个动态 Cordis 插件（Host + Client），在 DeepSeek Harness Web GUI 的「设置 → 消耗统计」中展示 Token 使用统计。只读重算会话日志，不写任何文件，能统计插件安装前的历史用量。

## 安装

> 前置：已安装 DeepSeek Harness 与 pnpm（`dsh plugin` 转发给 pnpm）。推荐使用带 Web 界面的 profile（如 `web`）。
...
- 点「刷新」按钮始终强制同步重扫。
- 标题副文案：加载时显示「正在统计会话日志…」，完成后显示「数据更新于 …」。

## 页面内容（设置 → 消耗统计）

1. **基础数据（全部历史）**：Token 总用量（含输入/输出拆分）、会话数量、最常用模型及占比。
2. **活跃热力图**：最近半年，GitHub 贡献图式布局——列为周、行为星期（一~日），带月份与星期标注；按非零日用量的四分位分 4 个绿色色阶（亮色/暗色主题各一套色板），悬停看明细。
3. **柱状图**：默认最近 7 天（可切 7/14/30 天），按模型堆叠分色展示每日用量，悬停看明细。
4. **模型用量**：左侧环形图（中央为 Token 总量），右侧列表前 5 个模型 + 第 6 行「其他」合并统计。
5. **刷新按钮**：强制重新扫描会话日志（右上角）。
...
- 样式全部使用 `--dsw-alias-*` 语义变量，跟随全局亮/暗主题。
