...
- **汇总数据（全部历史）** —— 计费输入 / 输出 Token、会话数量、最常用模型及其占比。
- **缓存命中率** —— `缓存读 ÷（未缓存输入 + 缓存读 + 缓存写）`，附读写绝对量。
- **覆盖度诊断** —— 成功 / 失败 / 待落盘会话数、数据路径（增量投影 / 全量重扫）、事件时间范围、重试次数与压缩用量。部分结果明确标注"部分数据"，绝不自称完整账单。
...
**日期口径声明**：日桶与导出均按 **UTC** 自然日（`YYYY-MM-DD`）。覆盖度卡片明确标注范围（"仅本机会话日志 · UTC"）。
