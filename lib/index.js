...
var PROJECTION_STATE_VERSION = 4;
...
async function scanFallback(deps, now) {
  ...
  const results = await mapConcurrent(sessions, 16, async (rec) => {
    ...
    if (!header) {
      return { status: "failed", err: "missing header" };
    }
    ...
    try {
      ...
      return { status: "failed", sessionId, err: String(err?.message ?? err) };
    }
    const events = snapshot && snapshot.events;
    if (!events || !events.length) {
      return { status: "ok", sessionId, title: null, state: initState(), depth: 0, counted: 0 };
    }
    ...
    return { status: "ok", sessionId, title, state, depth, counted };
  });
    ...
    const results = await mapConcurrent(sessions, 16, async (rec) => {
      ...
      if (seedLength > 0) {
        try {
          ...
          if (!events || !events.length) {
            return { status: "ok", id, value: initState(), depth };
          }
          const value = foldEvents(events, seedLength);
          return { status: "ok", id, value, depth };
        } catch (err) {
          return { status: "failed", id, err: String(err?.message ?? err) };
        }
      }
      try {
